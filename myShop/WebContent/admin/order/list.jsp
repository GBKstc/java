	...
	<HEAD>
		...
		<script type="text/javascript">
			var main = {
							     ...
							     for(var i=0;i<data.length;i++){
							    	 myHtml+=
							    		 '<tr onmouseover="this.style.backgroundColor = \'white\'"'+
												...
												'<td style="CURSOR: hand; HEIGHT: 22px" align="center"'+
													'width="17%">'+
													(data[i].state==0?"未付款":"已付款")+
												'</td>'+
												'<td align="center" style="HEIGHT: 22px">'+
													'<input type="button" value="订单详情" class="clickedElement" onclick="main.findOrderInfoByOid(\''+data[i].oid+'\')"/>'+
												'</td>'+
								
											'</tr>';
							     }
							     $("#DataGrid1").append(myHtml);
							     that.Pop();
							   }
							});
					},
					...
					findOrderInfoByOid:function(value){
						var that = this;
						$.ajax({
							   type: "POST",
							   url: "${pageContext.request.contextPath}/getOrderItems",
							   data: "oid="+value,
							   success: function(msg){
							     var data = JSON.parse(msg);
							     console.log(data);
							     var myHtml = '<tr id="showTableTitle">'+
										'<th width="20%">图片</th>'+
										'<th width="25%">商品</th>'+
										'<th width="20%">价格</th>'+
										'<th width="15%">数量</th>'+
							     ...
							     var num = 1;
							     for(var i=0;i<data.length;i++){
							    	 myHtml+='<tr style="text-align: center;">'+
										'<td>'+
						...
						<td class="ta_01" align="center" bgColor="#f5fafe">
							<table cellspacing="0" cellpadding="1" rules="all"
								...
								<tr onmouseover="this.style.backgroundColor = 'white'"
									...
									<td align="center" style="HEIGHT: 22px">
										<input type="button" value="订单详情" class="clickedElement" onclick="main.findOrderInfoByOid('fc86891e-5126-452e-932d-c4fe382ba73f')"/>
									</td>
