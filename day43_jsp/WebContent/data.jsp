<%@page import="java.util.Date"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>Insert title here</title>
</head>
<body>
<%out.print(new Date().toString());%>
<%
	/* Object obj = pageContext.getPage();
	HttpServletRequest sr = (HttpServletRequest)pageContext.getRequest(); */
	pageContext.removeAttribute("value");
	pageContext.setAttribute("value", "request",PageContext.REQUEST_SCOPE);
	pageContext.setAttribute("value", "session",PageContext.SESSION_SCOPE);
	pageContext.setAttribute("value", "servletContext",PageContext.APPLICATION_SCOPE);
%>
<%=pageContext.getAttribute("value")%>
<%=pageContext.getAttribute("value",PageContext.APPLICATION_SCOPE)%>
<%=pageContext.findAttribute("value")%>
</body>
</html>